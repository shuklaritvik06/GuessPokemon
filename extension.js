...
function activate(context) {
	let disposable = vscode.commands.registerCommand('pokemon-game.guessnow', async function () {
		...
		if(choice==="Abilities"){
			...
			const res = await vscode.window.showQuickPick(["Ready to Guess" , ...abilities],{
				prompt: "Guess the pokemon name",
				ignoreFocusOut: true,
		...
		if(choice==="Moves"){
			...
			const res = await vscode.window.showQuickPick(["Ready to Guess" , ...moves],{
				prompt: "Guess the pokemon name",
				ignoreFocusOut: true,
				placeHolder: "Pokemon Time 🎉🎉 "
			});
